...
struct Ext2File *ext2Open(char *fn,int32_t pNum){
    ...
    ext2File->fileDescriptor = vdiFile->fileDescriptor;
    ...
    partitionSeek(ext2File->partitionFile, ext2File->superBlock.firstDataBlock + (1024 << ext2File->superBlock.logBlockSize), SEEK_SET);
...
int32_t fetchBlock(struct Ext2File *f, uint32_t blockNum, void *buf){
    ...
    try {
        //Creating location variable for cleaner seek call
        off_t offset = f->superBlock.firstDataBlock + (blockNum * (1024 << f->superBlock.logBlockSize));

        //If valid index seek to block location
        partitionSeek(f->partitionFile, offset, f->partitionFile->startLoc);
        partitionRead(f->partitionFile, buf, 1024 << f->superBlock.logBlockSize);
...
int32_t writeBlock(struct Ext2File *f, uint32_t blockNum, void *buf){
    ...
    try {
        ...
        //If valid index seek to block location
        partitionSeek(f->partitionFile, offset, f->partitionFile->startLoc);
        partitionWrite(f->partitionFile, buf, 1024 << f->superBlock.logBlockSize);
